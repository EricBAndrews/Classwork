...
__global__ void do_block(int lda, int i, int j, int k,) {
  int row = i + threadIdx.x + (blockIdx.x * CUDA_BLOCK_SIZE);
  int col = j + threadIdx.y + (blockIdx.y * CUDA_BLOCK_SIZE);
  int lBound = min(CUDA_BLOCK_SIZE, lda-row);
  for (int l = 0; l < lBound; ++l) {
    matC[row][col] += matA[row][l] * matB[l][col];
  }
}
...
void square_dgemm (int lda, double* A, double* B, double* C) {
  ...
  cudaMalloc((double*)&matA, matSize);
  cudaMalloc((double*)&matB, matSize);
  cudaMalloc((double*)&matC, matSize);
  ...
  for (int i = 0; i < lda; i += BLOCK_SIZE) {
	...
	do_block<<<dimBlock, dimGrid>>>(lda, i, j, k, M);
