...
void square_dgemm (int lda, double* A, double* B, double* C) {
  ...
  for (int i = 0; i < lda; i += BLOCK_SIZE) {
	...
	if (BLOCK_SIZE <= lda-i && BLOCK_SIZE <= lda-j && BLOCK_SIZE <= lda-k) {
	  ...
	  double* aBase = A + i + k*lda;
	  double* bBase = B + k + j*lda;
	  double* cBase = C + i + j*lda;
	  __m256d c0 = _mm256_loadu_pd(cBase + 0*lda);
	  __m256d c1 = _mm256_loadu_pd(cBase + 1*lda);
	  __m256d c2 = _mm256_loadu_pd(cBase + 2*lda);
	  __m256d c3 = _mm256_loadu_pd(cBase + 3*lda);
	  __m256d a, b0, b1, b2, b3;
	  for (int l = 0; l < 4; ++l) {
	    a = _mm256_loadu_pd(aBase + l*lda);
	    b0 = _mm256_set1_pd(bBase[l + 0*lda]);
	    b1 = _mm256_set1_pd(bBase[l + 1*lda]);
	    b2 = _mm256_set1_pd(bBase[l + 2*lda]);
	    b3 = _mm256_set1_pd(bBase[l + 3*lda]);
	    c0 = _mm256_add_pd(c0, _mm256_mul_pd(a, b0));
	    c1 = _mm256_add_pd(c1, _mm256_mul_pd(a, b1));
	    c2 = _mm256_add_pd(c2, _mm256_mul_pd(a, b2));
	    c3 = _mm256_add_pd(c3, _mm256_mul_pd(a, b3));
	  }
	  _mm256_storeu_pd(cBase + 0*lda, c0);
	  _mm256_storeu_pd(cBase + 1*lda, c1);
	  _mm256_storeu_pd(cBase + 2*lda, c2);
	  _mm256_storeu_pd(cBase + 3*lda, c3);
	}
	else {
