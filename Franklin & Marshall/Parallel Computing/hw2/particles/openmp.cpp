...
#pragma omp for
      for (int i = 0; i < n; i++) {
        int curBin = binNum(particles[i], bpr);
        omp_set_lock(&(locks[curBin]));
        bins[curBin].push_back(particles + i);
        omp_unset_lock(&(locks[curBin]));
	// bins[binNum(particles[i],bpr)].push_back(particles + i);
      ...
      //  compute forces
      //
#pragma omp for reduction (+:davg) reduction (+:navg)
      for (int p = 0; p < n; p++) {
